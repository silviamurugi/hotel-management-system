        ...
        if (user.verificationCode === code) {
            ...
            try {
                await sendEmail(
                    ...
                    `Hello ${user.last_name}, your account has been verified. You can now log in and use all features.`,
                    `<div>
...
export const loginUserController = async (req: Request, res: Response) => {
    try {
        ...
        const userMatch = await bycrypt.compareSync(user.password, userExist.password_hash)
        if (!userMatch) {
